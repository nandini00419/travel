import streamlit as st
import pandas as pd
import sqlite3
import json
from datetime import datetime, timedelta
import os

def main():
    st.set_page_config(
        page_title="Travel Assistant Admin Dashboard",
        page_icon="📊",
        layout="wide"
    )
    
    st.title("📊 Travel Assistant Admin Dashboard")
    st.markdown("---")
    
    # Sidebar for navigation
    with st.sidebar:
        st.header("📈 Dashboard Sections")
        section = st.selectbox(
            "Choose a section:",
            ["Overview", "User Analytics", "System Statistics", "Recent Activity", "Database Tables"]
        )
    
    if section == "Overview":
        show_overview()
    elif section == "User Analytics":
        show_user_analytics()
    elif section == "System Statistics":
        show_system_statistics()
    elif section == "Recent Activity":
        show_recent_activity()
    elif section == "Database Tables":
        show_database_tables()

def get_db_connection():
    """Get database connection with fallback paths"""
    db_paths = [
        "travel/monitoring.db",
        "monitoring.db",
        "travel/configuration/monitoring.db"
    ]
    
    for db_path in db_paths:
        if os.path.exists(db_path):
            try:
                return sqlite3.connect(db_path)
            except Exception as e:
                st.warning(f"Could not connect to {db_path}: {e}")
                continue
    
    st.error("Could not find monitoring database")
    return None

def get_postgres_connection():
    """Get PostgreSQL connection for main database"""
    try:
        import psycopg2
        import os
        
        # Try to get from environment variables
        conn_params = {
            'host': os.getenv('DB_HOST'),
            'database': os.getenv('DB_NAME'),
            'user': os.getenv('DB_USER'),
            'password': os.getenv('DB_PASSWORD'),
            'port': int(os.getenv('DB_PORT', 5432)),
            'sslmode': os.getenv('DB_SSLMODE', 'require')
        }
        
        # Check if all required parameters are present
        if not all([conn_params['host'], conn_params['database'], conn_params['user'], conn_params['password']]):
            return None
            
        return psycopg2.connect(**conn_params)
    except Exception as e:
        st.warning(f"Could not connect to PostgreSQL: {e}")
        return None

def show_overview():
    """Show overview metrics"""
    st.header("📊 System Overview")
    
    col1, col2, col3, col4 = st.columns(4)
    
    try:
        conn = get_db_connection()
        if not conn:
            st.error("Could not connect to monitoring database")
            return
        
        # Get system stats
        cursor = conn.cursor()
        
        # Total users
        cursor.execute("SELECT COUNT(DISTINCT user_id) FROM logs")
        total_users = cursor.fetchone()[0]
        
        # Messages today
        cursor.execute("""
            SELECT COUNT(*) FROM logs 
            WHERE log_type = 'user_input' AND date(timestamp) = date('now')
        """)
        messages_today = cursor.fetchone()[0]
        
        # Errors today
        cursor.execute("SELECT COUNT(*) FROM error_logs WHERE date(timestamp) = date('now')")
        errors_today = cursor.fetchone()[0]
        
        # Average response time
        cursor.execute("SELECT AVG(response_time_ms) FROM api_calls WHERE response_time_ms IS NOT NULL")
        avg_response = cursor.fetchone()[0]
        
        with col1:
            st.metric("Total Users", total_users)
        
        with col2:
            st.metric("Messages Today", messages_today)
        
        with col3:
            st.metric("Errors Today", errors_today)
        
        with col4:
            st.metric("Avg Response Time", f"{avg_response:.0f}ms" if avg_response else "N/A")
        
        conn.close()
    
    except Exception as e:
        st.error(f"Error loading overview: {e}")
    
    # Recent activity chart
    st.subheader("📈 Recent Activity (Last 7 Days)")
    try:
        conn = get_db_connection()
        if conn:
            query = """
            SELECT 
                date(timestamp) as date,
                COUNT(*) as activity_count
            FROM user_activity 
            WHERE timestamp > datetime('now', '-7 days')
            GROUP BY date(timestamp)
            ORDER BY date(timestamp)
            """
            
            df = pd.read_sql_query(query, conn)
            conn.close()
            
            if not df.empty:
                st.line_chart(df.set_index('date'))
            else:
                st.info("No recent activity data available")
        else:
            st.info("No monitoring database available")
            
    except Exception as e:
        st.error(f"Error loading activity chart: {e}")

def show_user_analytics():
    """Show detailed user analytics"""
    st.header("👥 User Analytics")
    
    try:
        conn = get_db_connection()
        if not conn:
            st.error("Could not connect to monitoring database")
            return
        
        # Get all unique users
        users_query = """
        SELECT DISTINCT user_id, 
               MIN(timestamp) as first_seen,
               MAX(timestamp) as last_seen,
               COUNT(*) as total_activities
        FROM user_activity 
        GROUP BY user_id
        ORDER BY first_seen DESC
        """
        
        users_df = pd.read_sql_query(users_query, conn)
        
        if not users_df.empty:
            st.subheader("📋 All Users")
            
            # Format the dataframe for better display
            users_df['first_seen'] = pd.to_datetime(users_df['first_seen']).dt.strftime('%Y-%m-%d %H:%M')
            users_df['last_seen'] = pd.to_datetime(users_df['last_seen']).dt.strftime('%Y-%m-%d %H:%M')
            
            st.dataframe(
                users_df,
                use_container_width=True,
                column_config={
                    "user_id": "User ID",
                    "first_seen": "First Seen",
                    "last_seen": "Last Seen", 
                    "total_activities": "Total Activities"
                }
            )
            
            # User details
            st.subheader("🔍 User Details")
            selected_user = st.selectbox(
                "Select a user to view details:",
                users_df['user_id'].tolist()
            )
            
            if selected_user:
                show_user_details(selected_user, conn)
        else:
            st.info("No user data available")
            
        conn.close()
        
    except Exception as e:
        st.error(f"Error loading user analytics: {e}")

def show_user_details(user_id, conn):
    """Show detailed information for a specific user"""
    try:
        col1, col2 = st.columns(2)
        
        with col1:
            st.subheader("📊 User Statistics")
            
            cursor = conn.cursor()
            
            # Total messages
            cursor.execute("SELECT COUNT(*) FROM logs WHERE user_id = ? AND log_type = 'user_input'", (user_id,))
            total_messages = cursor.fetchone()[0]
            
            # Total AI responses
            cursor.execute("SELECT COUNT(*) FROM logs WHERE user_id = ? AND log_type = 'ai_response'", (user_id,))
            total_responses = cursor.fetchone()[0]
            
            # Total errors
            cursor.execute("SELECT COUNT(*) FROM error_logs WHERE user_id = ?", (user_id,))
            total_errors = cursor.fetchone()[0]
            
            # Recent activity (24h)
            cursor.execute("""
                SELECT COUNT(*) FROM user_activity 
                WHERE user_id = ? AND timestamp > datetime('now', '-1 day')
            """, (user_id,))
            recent_activity = cursor.fetchone()[0]
            
            st.metric("Total Messages", total_messages)
            st.metric("Total AI Responses", total_responses)
            st.metric("Total Errors", total_errors)
            st.metric("Recent Activity (24h)", recent_activity)
        
        with col2:
            st.subheader("🗄️ Database Analytics")
            
            # Try to get PostgreSQL data
            pg_conn = get_postgres_connection()
            if pg_conn:
                cursor = pg_conn.cursor()
                
                # Total conversations
                cursor.execute("SELECT COUNT(*) FROM conversations WHERE user_id = %s", (user_id,))
                total_conversations = cursor.fetchone()[0]
                
                st.metric("Total Conversations", total_conversations)
                
                pg_conn.close()
            else:
                st.info("PostgreSQL database not available")
        
        # Recent activity for this user
        st.subheader("📝 Recent Activity")
        cursor = conn.cursor()
        
        activity_query = """
        SELECT timestamp, action_type, details
        FROM user_activity 
        WHERE user_id = ?
        ORDER BY timestamp DESC
        LIMIT 10
        """
        
        activity_df = pd.read_sql_query(activity_query, conn, params=(user_id,))
        
        if not activity_df.empty:
            activity_df['timestamp'] = pd.to_datetime(activity_df['timestamp']).dt.strftime('%Y-%m-%d %H:%M:%S')
            st.dataframe(activity_df, use_container_width=True)
        else:
            st.info("No recent activity for this user")
            
    except Exception as e:
        st.error(f"Error loading user details: {e}")

def show_system_statistics():
    """Show system-wide statistics"""
    st.header("⚙️ System Statistics")
    
    try:
        conn = get_db_connection()
        if not conn:
            st.error("Could not connect to monitoring database")
            return
        
        # API calls statistics
        st.subheader("🔌 API Calls")
        api_query = """
        SELECT 
            api_service,
            COUNT(*) as total_calls,
            AVG(response_time_ms) as avg_response_time,
            COUNT(CASE WHEN status_code >= 200 AND status_code < 300 THEN 1 END) as successful_calls,
            COUNT(CASE WHEN status_code >= 400 THEN 1 END) as error_calls
        FROM api_calls 
        GROUP BY api_service
        """
        
        api_df = pd.read_sql_query(api_query, conn)
        
        if not api_df.empty:
            api_df['avg_response_time'] = api_df['avg_response_time'].round(2)
            st.dataframe(api_df, use_container_width=True)
        else:
            st.info("No API call data available")
        
        # Error statistics
        st.subheader("❌ Error Statistics")
        error_query = """
        SELECT 
            error_type,
            COUNT(*) as error_count,
            MAX(timestamp) as last_occurrence
        FROM error_logs 
        GROUP BY error_type
        ORDER BY error_count DESC
        """
        
        error_df = pd.read_sql_query(error_query, conn)
        
        if not error_df.empty:
            error_df['last_occurrence'] = pd.to_datetime(error_df['last_occurrence']).dt.strftime('%Y-%m-%d %H:%M')
            st.dataframe(error_df, use_container_width=True)
        else:
            st.info("No error data available")
        
        conn.close()
        
    except Exception as e:
        st.error(f"Error loading system statistics: {e}")

def show_recent_activity():
    """Show recent activity logs"""
    st.header("📝 Recent Activity")
    
    try:
        conn = get_db_connection()
        if not conn:
            st.error("Could not connect to monitoring database")
            return
        
        # Recent logs
        logs_query = """
        SELECT timestamp, user_id, log_type, message, level
        FROM logs 
        ORDER BY timestamp DESC
        LIMIT 50
        """
        
        logs_df = pd.read_sql_query(logs_query, conn)
        
        if not logs_df.empty:
            logs_df['timestamp'] = pd.to_datetime(logs_df['timestamp']).dt.strftime('%Y-%m-%d %H:%M:%S')
            
            # Filter options
            log_level = st.selectbox("Filter by log level:", ["All", "INFO", "ERROR", "WARNING"])
            if log_level != "All":
                logs_df = logs_df[logs_df['level'] == log_level]
            
            st.dataframe(logs_df, use_container_width=True)
        else:
            st.info("No recent logs available")
        
        conn.close()
        
    except Exception as e:
        st.error(f"Error loading recent activity: {e}")

def show_database_tables():
    """Show database table information"""
    st.header("🗄️ Database Tables")
    
    # Show SQLite tables
    st.subheader("📊 SQLite Monitoring Database")
    try:
        conn = get_db_connection()
        if conn:
            cursor = conn.cursor()
            
            # Get table information
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
            tables = cursor.fetchall()
            
            if tables:
                for table_name, in tables:
                    with st.expander(f"📊 {table_name}"):
                        # Get row count
                        cursor.execute(f"SELECT COUNT(*) FROM {table_name}")
                        row_count = cursor.fetchone()[0]
                        st.write(f"**Rows:** {row_count}")
                        
                        # Show sample data
                        if st.button(f"Show Sample Data", key=f"sqlite_sample_{table_name}"):
                            cursor.execute(f"SELECT * FROM {table_name} LIMIT 5")
                            sample_data = cursor.fetchall()
                            
                            if sample_data:
                                # Get column names
                                cursor.execute(f"PRAGMA table_info({table_name})")
                                columns_info = cursor.fetchall()
                                column_names = [col[1] for col in columns_info]
                                
                                sample_df = pd.DataFrame(sample_data, columns=column_names)
                                st.dataframe(sample_df, use_container_width=True)
                            else:
                                st.info("No data in this table")
            else:
                st.info("No tables found in SQLite database")
            
            conn.close()
        else:
            st.info("SQLite monitoring database not available")
    except Exception as e:
        st.error(f"Error loading SQLite tables: {e}")
    
    # Show PostgreSQL tables
    st.subheader("🐘 PostgreSQL Main Database")
    try:
        conn = get_postgres_connection()
        if conn:
            cursor = conn.cursor()
            
            # Get table information
            cursor.execute("""
                SELECT table_name, 
                       (SELECT COUNT(*) FROM information_schema.columns WHERE table_name = t.table_name) as column_count
                FROM information_schema.tables t
                WHERE table_schema = 'public'
                ORDER BY table_name
            """)
            
            tables = cursor.fetchall()
            
            if tables:
                for table_name, column_count in tables:
                    with st.expander(f"📊 {table_name} ({column_count} columns)"):
                        # Get row count
                        cursor.execute(f"SELECT COUNT(*) FROM {table_name}")
                        row_count = cursor.fetchone()[0]
                        st.write(f"**Rows:** {row_count}")
                        
                        # Get column information
                        cursor.execute(f"""
                            SELECT column_name, data_type, is_nullable
                            FROM information_schema.columns 
                            WHERE table_name = '{table_name}'
                            ORDER BY ordinal_position
                        """)
                        
                        columns = cursor.fetchall()
                        
                        if columns:
                            col_df = pd.DataFrame(columns, columns=['Column', 'Type', 'Nullable'])
                            st.dataframe(col_df, use_container_width=True)
                        
                        # Show sample data
                        if st.button(f"Show Sample Data", key=f"pg_sample_{table_name}"):
                            cursor.execute(f"SELECT * FROM {table_name} LIMIT 5")
                            sample_data = cursor.fetchall()
                            
                            if sample_data:
                                # Get column names
                                cursor.execute(f"""
                                    SELECT column_name 
                                    FROM information_schema.columns 
                                    WHERE table_name = '{table_name}'
                                    ORDER BY ordinal_position
                                """)
                                column_names = [row[0] for row in cursor.fetchall()]
                                
                                sample_df = pd.DataFrame(sample_data, columns=column_names)
                                st.dataframe(sample_df, use_container_width=True)
                            else:
                                st.info("No data in this table")
            else:
                st.info("No tables found in PostgreSQL database")
            
            conn.close()
        else:
            st.info("PostgreSQL database not available")
    except Exception as e:
        st.error(f"Error loading PostgreSQL tables: {e}")

if __name__ == "__main__":
    main()
